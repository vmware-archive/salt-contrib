...
def run(cmd='shell:logout'):
    ...
    if ret and ret[0].startswith('client: JAVA_HOME not set'):
        ret.pop(0)
    return ret
...
def feature_install(feature, version='', bundles='', wait4bundles=5):
    ...
    if not is_feature_installed(feature, version):
        feature_refreshurls()
