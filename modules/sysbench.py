'''
The module 'sysbench' is used to analyse / measure
...
def cpu(option):
    ...
    USAGE: salt \* sysbench.cpu run - gives the total execution time
           salt \* sysbench.cpu verbose - gives a brief output
    '''
...
def threads(option):
    ...
    scheduling. 
    USAGE: salt \* sysbench.threads run
...
def memory(option):
    ...
    USAGE: salt \* sysbench.memory run
...
def fileio(option):
    ...
    USAGE: salt \* sysbench.fileio run
