...
def execute(ctid=None,option=None):
	'''
...
'''
	...
	if ret:
		output = _runCommand("exec",ctid,option)
		return output
	else:
		return error

def start(ctid=None,option=None):
	'''
Start a container.
...
'''
	...
	if ret:
		output = _runCommand("start",ctid,option)
		return output
	else:
		return error

def stop(ctid=None,option=None):
	'''
Stop a container.
...
'''
	...
	if ret:
		output = _runCommand("stop",ctid,option)
		return output
	else:
		return error

def restart(ctid=None,option=None):
	'''
Restart a container.
...
'''
	...
	if ret:
		output = _runCommand("restart",ctid,option)
		return output
	else:
		return error
...
def _runCommand(command,ctid,option):
	'''
Use salt to run the command and output.
'''
	if option is None:
		cmd = 'vzctl %s %s' %(command,ctid)
		out = __salt__['cmd.run'](cmd)
		return out
	else:
		cmd = 'vzctl %s %s %s' %(command,ctid,option)
		out = __salt__['cmd.run'](cmd)
		return out
