...
def sysmem(human_readable=False):
    '''
    .. versionadded:: 2016.11.1
	
    Returns the amount of memory the system has.
    ...
    .. code-block:: bash
        ...
        salt '*' status.sysmem
        salt '*' status.sysmem human_readable=True
    '''
    with salt.utils.winapi.Com():
        totalmem = 0
        wmi_obj = wmi.WMI()
        ...
        for module in result:
            mem = int(module.capacity)
            totalmem = totalmem + mem

        if human_readable:
            return _byte_calc(totalmem)
        return totalmem
