...
def __virtual__():
    '''
    Load if the module tomcat_manager exists
    '''
    
    return 'tomcat_manager' if 'tomcat_manager.status' in __salt__ else False
...
def war_deployed(name, war, url='http://localhost:8080/manager', __env__='base'):
    ...
    if deploy_res.startswith('OK'):
        ret['result'] = True
        ret['comment'] = __salt__['tomcat_manager.ls']()[name]
        ret['changes']['deploy'] = 'deployed {0} in version {1}'.format(name, version)
    else:
        ret['result'] = False
